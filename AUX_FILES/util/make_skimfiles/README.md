...
1. make_skimmed_rootfile.C : Main script written in C++. It creates a skim file for a given run. The loose analysis cuts, reconstructed kinematics, and list of ROOT variables to be copied to the skimmed files are defined within. Run-dependent kinematic settings are read from this checkout's HCANA database files.
