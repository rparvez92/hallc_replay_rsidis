...
require SWIF2, Slurm, hcana, or the analyzer executable.
...
From the `hallc_replay_rsidis` repository root:

```bash
script_dir="$PWD/AUX_FILES/util/scaler_event_check"

"$script_dir/run_scaler_event_check.sh" \
  sample \
  "$script_dir/sample_scaler_event_check_runlist.csv" \
  /lustre24/expphy/volatile/hallc/c-rsidis/pdbforce/replay/pass1_QA_scdisc \
  "$script_dir/results/scaler_event_check_sample.csv" \
  "$script_dir/results/scaler_event_check_sample_flagged.csv"
```
...
## Regenerate the original phase run lists

The original Phase 1 and Phase 2 lists come from their official DAT files:
